...
- **Pre-release**: `X.Y.Z-beta` or `X.Y.Z-beta.N` (e.g., `0.3.0-beta`, `0.3.0-beta.2`)
...
```bash
npm install grafema@latest   # Stable
npm install grafema@beta     # Pre-release
npm install grafema@0.3.0    # Specific version
```

### Package structure (v0.3+)

The unified `grafema` package includes CLI, MCP server, and platform-specific binaries:
- `grafema` — main entry point (CLI + MCP + query layer)
- `@grafema/grafema-darwin-arm64` — macOS ARM binaries (optional dep)
...
```bash
npm unpublish grafema@0.3.0-beta
```

### Or deprecate
```bash
npm deprecate grafema@0.3.0-beta "Use 0.2.13 instead"
```
