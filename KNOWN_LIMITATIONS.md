...
- ~~**RFDB stale socket**~~ — Fixed in v0.3.31: `grafema doctor` removes a stale `rfdb.sock` when connect fails (nothing listening), so a crashed server no longer blocks a clean restart. A live but unresponsive server is reported as a warning and its socket is preserved (REG-1129, #373).
