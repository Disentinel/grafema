...
- [ ] **`--clear` как дефолт или рабочий incremental** — при повторном analyze без `--clear` дубликаты по semanticID. Либо `--clear` default, либо допилить/протестить incremental. Enricher-результаты при `--clear` теряются → нужен auto-rerun enrichers.
- [ ] **Barrel file re-export resolution** (GAP-5) — `export { X } from './internal'` не резолвится через barrel files. Примеры: jodit-startup-service exports `AuthenticatedRequest`, `CrontabConfig` etc. через barrel, resolver не находит. Тестить на Jodit до 100%.
...
| 10 | Barrel file re-exports (GAP-5) | BLOCKER | Fix resolver, test on Jodit until 100% |
