...
- [x] **`download-platform-binaries.sh` скачивает бинарники** — dry run на `binaries-v0.3.0-test3`: 6 binaries, 3 platforms OK. Скрипт обновлён: quality only required (orchestrator+rfdb-server), optional не попадают в platform packages. linux-arm64 исключён (non-blocker).
- [x] **Platform packages содержат рабочие бинарники** — darwin-arm64 Mach-O ARM64, darwin-x64 Mach-O x86_64, linux-x64 ELF x86-64. По 2 бинарника (orchestrator+rfdb-server) каждый.
- [ ] **`npm install grafema` на чистой машине** — тест на darwin-arm64
- [ ] **`npm install grafema` на чистой машине** — тест на linux-x64
- [x] **`npx grafema --version`** — выводит `0.2.12-beta` (version bump при release)
- [x] **`npx grafema-mcp`** — MCP server stdio-based, запускается (нет --version, ожидаемо)
...
- [x] **Platform packages** — Haskell бинарники убраны из `bin/`, download скрипт обновлён (только required). Верифицировано 2026-03-14.
...
## 11. Jodit Dogfooding Findings (Phase 1.5 results)

### Выполнено
- [x] Multi-repo analysis: 1306 files, 231K nodes, 513K edges
- [x] Cross-repo resolution: services + aliases (5 connections resolved)
- [x] `who` command: 3 strategies (CALL scan + incoming edges + IMPORT_BINDING)
- [x] GAP-4 fix: `metadata.base` on PROPERTY_ACCESS (57.7% coverage, was 0%)
- [x] 3 enricher-плагина: Express routes (202), Redis (48), Events (613 edges)
- [x] Exploration log: architecture diagrams, side effects map, 6 gaps tracked

### Критичные для релиза (БЛОКЕР)

- [x] **Incremental analysis стабилен** — фикс `8027478d` (compact flush + threshold=1 + tombstone ordering между фазами). Верифицировано 2026-03-14: 3 прогона подряд на ts-barrel-exports fixture → 61 nodes / 105 edges каждый раз, дубликатов нет.
- [x] **Barrel file re-export resolution** (GAP-5) — фикс `8027478d` (ImportResolution.hs chain following). Верифицировано 2026-03-14: named re-exports (`export { X } from`) и star re-exports (`export * from`) резолвятся к source файлам. 5/5 символов resolved на ts-barrel-exports fixture.

### Backlog из dogfooding (post-v0.3 или v0.3 если успеем)

| # | Предложение | Приоритет | Решение |
|---|------------|-----------|---------|
| 1 | `grafema switch <path>` — MCP переключение проекта | HIGH | CLI command + MCP handler, переподключение к другому .grafema/ |
| 2 | `grafema enrich` — ручной запуск enricher-ов из `.grafema/plugins/` | HIGH | CLI command, сканирует plugins/, запускает все .mjs/.js |
| 3 | Enricher-ы в pipeline — авто-запуск после analyze, persistence при `--clear` | HIGH | `plugins:` в конфиге запускает enrichers после analysis |
| 4 | `get_documentation topic="plugins"` — документация emit protocol | MED | Добавить topic в MCP |
| 5 | Builtin enrichers для Express/Redis/EventEmitter/Mongoose | HIGH | Auto-detect external modules → download enricher → run. Конфиг контролирует разрешение. Хранить: (a) в GitHub releases как отдельные файлы, (b) или зашить в анализатор если легковесные |
| 6 | Event name extraction (.on('name') → string arg) | MED | Нужны PASSES_ARGUMENT edges, enricher извлекает 1-й аргумент |
| 7 | `<obj>` resolution improvement (42.3% remaining) | LOW | Deeper AST: chain members, ternary, destructured |
| 8 | ~~discover_services auto-detection~~ | SKIP | JS-specific, rely on AI/user in early version |
| 9 | ~~Alias autodetect~~ | SKIP | Magic, won't work reliably |
| 10 | ~~Barrel file re-exports (GAP-5)~~ | DONE | Fixed in `8027478d`, verified 2026-03-14 |
| 11 | `grafema init-plugin <name>` — scaffold generator | LOW | Post-v0.3 |
| 12 | Auto-download enrichers for detected frameworks | HIGH | Detect external module → download plugin → run. Config flag: `auto_enrich: true/false` |
