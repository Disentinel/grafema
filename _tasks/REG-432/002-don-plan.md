# Don's Implementation Plan: Socket Connection Analysis (REG-432)
...
**`socket:connection`** (client-side, outgoing connections)
- Protocol: `unix` | `tcp`
- For Unix: `path` (e.g., `/tmp/app.sock`, `.grafema/rfdb.sock`)
- For TCP: `host`, `port`
- Library: `net`, `ipc`, or custom wrapper

**`socket:server`** (server-side listeners)
- Protocol: `unix` | `tcp`
- For Unix: `path`
- For TCP: `host`, `port`
...
- **`INTERACTS_WITH`**: `socket:connection` → `socket:server` (same as HTTP pattern)
- **`CONTAINS`**: `MODULE` → `socket:connection` / `socket:server`
- **`MAKES_REQUEST`**: `FUNCTION` → `socket:connection`, `CALL` → `socket:connection`
...
Add node record interfaces:
```typescript
// Socket connection node (client)
export interface SocketConnectionNodeRecord extends BaseNodeRecord {
...
// Socket server node
export interface SocketServerNodeRecord extends BaseNodeRecord {
  type: 'socket:server';
  protocol: 'unix' | 'tcp';
  path?: string;           // Unix socket path
  host?: string;           // TCP host
...
export function isSideEffectType(nodeType: string): boolean {
  ...
  return ns === 'db' || ns === 'fs' || ns === 'net' || ns === 'socket';
}
```
...
```typescript
/**
 * SocketConnectionEnricher - links socket:connection (client) to socket:server
 *
...
export class SocketConnectionEnricher extends Plugin {
  ...
  async execute(context: PluginContext): Promise<PluginResult> {
    // 1. Query all socket:connection nodes
    // 2. Query all socket:server nodes
...
describe('SocketAnalyzer', () => {
  ...
  it('detects Unix socket client (net.connect with path)', async () => {
    ...
    // Run analysis
    // Assert socket:connection node exists with protocol='unix', path='/tmp/app.sock'
  });

  it('detects TCP socket client (net.connect with port/host)', async () => {
    // Setup test project with net.connect(3000, 'localhost')
    // Assert socket:connection node exists with protocol='tcp', port=3000, host='localhost'
  });
  ...
  it('detects Unix socket server (net.createServer().listen(path))', async () => {
    ...
    // Assert socket:server node exists with protocol='unix', path='/tmp/app.sock'
  });

  it('detects TCP socket server (net.createServer().listen(port))', async () => {
    // Assert socket:server node exists with protocol='tcp', port=3000
  });

  it('creates MAKES_REQUEST edge from CALL to socket:connection', async () => {
    // Verify edge exists: CALL (net.connect) → socket:connection
  });

  it('creates MAKES_REQUEST edge from FUNCTION to socket:connection', async () => {
    // Verify edge exists: FUNCTION → socket:connection
  });
});
```
...
Create `/Users/vadim/grafema-worker-9/test/fixtures/08-socket-connections/`:

```
08-socket-connections/
├── package.json
...
3. Expected result:
   - `socket:connection` node in `RFDBServerBackend.ts`
...
4. Check for enrichment edge:
   ...
   node packages/cli/dist/cli.js query -d "
     connection(client, server) :-
       ...
       node(client, 'socket:connection', _),
       node(server, 'socket:server', _).
   "
   ```
...
**Time Complexity:**
- SocketAnalyzer: O(M × N) where M = number of modules, N = average AST nodes per module
- Matches FetchAnalyzer: linear scan of all modules, AST traversal is linear in code size
- SocketConnectionEnricher: O(C × S) where C = socket:connection nodes, S = socket:server nodes
- Expected: C and S are small (typically <100 each in large projects)

**Space Complexity:**
- O(M) for module iteration (one at a time)
- O(N) for nodes/edges batch arrays during analysis
- Matches existing analyzer pattern
...
3. **SocketAnalyzer** (creates socket:connection, socket:server)
...
- [x] Node types defined in types package
...
5. `/Users/vadim/grafema-worker-9/test/fixtures/08-socket-connections/` (5 files)

**Modified files:**
1. `/Users/vadim/grafema-worker-9/packages/types/src/nodes.ts` (+50 lines: node types, interfaces)
