...
-- Adapted from core-v3/Analysis.Types for Haskell-specific analysis.
...
data DeferredKind
  ...
  | TypeResolve
  | AliasResolve
  | ModuleResolve    -- ^ Haskell module lookup (qualified imports, re-exports)
  deriving (Show, Eq)
...
-- ── Scope Types (Haskell-specific) ────────────────────────────────────

data ScopeKind
  = ModuleScope       -- ^ Top-level module scope
  | FunctionScope     -- ^ Function equation / match group
  | WhereScope        -- ^ where clause (local bindings)
  | LetScope          -- ^ let ... in expression
  | CaseScope         -- ^ case alternative
  | DoScope           -- ^ do-notation block
  | LambdaScope       -- ^ lambda expression
  | ClassScope        -- ^ type class declaration body
  | InstanceScope     -- ^ type class instance declaration body
  deriving (Show, Eq)

data DeclKind
  = DeclFunction      -- ^ Top-level or local function binding
  | DeclVariable      -- ^ Pattern-bound variable
  | DeclParameter     -- ^ Function parameter
  | DeclConstructor   -- ^ Data constructor
  | DeclTypeClass     -- ^ Type class declaration
  | DeclInstance       -- ^ Type class instance
  | DeclTypeSig       -- ^ Type signature
  | DeclDataType      -- ^ data / newtype declaration
  | DeclTypeAlias     -- ^ type synonym
  | DeclImport        -- ^ Imported name
  deriving (Show, Eq)
...
deferredKindText ModuleResolve  = "module_resolve"
