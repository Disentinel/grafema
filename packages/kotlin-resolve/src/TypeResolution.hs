...
-- | Kotlin built-in types that should not be resolved to class nodes.
-- This includes primitive-equivalent types and common stdlib types.
...
builtinTypes = Set.fromList
  ...
  , "Pair", "Triple"
  -- Java-style primitive names (in case they appear in metadata)
...
normalizeType raw =
  ...
  in if T.null withoutNullable
       ...
       then Nothing
     else if T.isInfixOf "?" withoutNullable   -- remaining wildcards
       then Nothing
