...
-- | Analyze a pre-parsed C/C++ AST (received as JSON from the orchestrator).
analyzeAst :: T.Text -> CppFile -> FileAnalysis
analyzeAst file ast =
  let moduleId = makeModuleId file
      isHeader = isHeaderFile file
  in runAnalyzer file moduleId isHeader (walkFile ast)

-- | Check if a file is a header file.
isHeaderFile :: T.Text -> Bool
isHeaderFile path =
  T.isSuffixOf ".h" path
