...
-- Adapted from go-analyzer's Analysis.Types for C/C++-specific analysis.
...
data DeferredKind
  = IncludeResolve         -- ^ #include directive cross-file resolution
  | CallResolve            -- ^ function/method call resolution
  | TypeResolve            -- ^ type reference resolution
  | InheritanceResolve     -- ^ base class/interface resolution
  | TemplateResolve        -- ^ template instantiation resolution
  deriving (Show, Eq)
...
-- ── Scope Types (C/C++-specific) ──────────────────────────────────────

data ScopeKind
  = ModuleScope           -- ^ top-level file scope (translation unit)
  | FunctionScope         -- ^ function body
  | BlockScope            -- ^ any block ({ })
  | ClassScope            -- ^ class/struct/union body
  | NamespaceScope        -- ^ namespace body
  | TemplateScope         -- ^ template parameter scope
  | LambdaScope           -- ^ lambda body
  | TryScope              -- ^ try block scope
  | LoopScope             -- ^ loop body scope
  deriving (Show, Eq)
...
data AccessSpec
  ...
  | Protected
  | Private
  | DefaultAccess         -- ^ no explicit specifier (struct default = public, class = private)
  deriving (Show, Eq)
...
deferredKindText IncludeResolve     = "include_resolve"
...
deferredKindText TemplateResolve    = "template_resolve"
