...
-- | Analyze a pre-parsed Obj-C AST (received as JSON from the orchestrator).
analyzeAst :: T.Text -> ObjcFile -> FileAnalysis
analyzeAst file ast =
  let moduleId = makeModuleId file
  in runAnalyzer file moduleId (walkFile ast)
