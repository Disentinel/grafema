...
-- HaskellLocalCalls / HaskellLocalRefs RETIRED — same-file CALLS/READS_FROM
-- resolution now runs in-engine via the @stdlib/haskell_local_* .dl packs
-- (rfdb-server derive/stdlib), with fixture tests there. Only the cross-file
-- arms (ImportResolution / CrossModuleCalls) remain in this suite.
