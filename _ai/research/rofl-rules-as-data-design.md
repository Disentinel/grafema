...
| **Третья группа — факты о фактах, а не о правилах** | `in_perspective/2`, `asserted_by/2`, `derived_by/3` | аудит подделок, `boot.rofl:33-34` |
...
Механику вытеснения я проверил живой пробой, а не чтением по смыслу. Проба —
`/tmp/probe_supersede_granularity.rs`, запуск из `packages/rfdb-server`:
...
0 failed`. Положительный контроль зашит в подготовку: до вытеснения проба требует, чтобы оба
соседних факта были живы.
...
получила типизированный отказ `E-CAP-001 missing capability 'record-vocabulary'`, а словарь записи —
базовая пятёрка `node`/`ty`/`edge`/`incoming`/`attr` плюс `supersedes` (`facts/lsm.rs:423-429`). И
...
* **boot.rofl не загружен.** Вывести `unstratified/1` некому, и приёмку решает `derive/stratify.rs` —
  то есть RFDB отвергает то, что эталон принимает. Это осознанное расхождение в сторону ОТКАЗА, оно
  записано в разделе 9, пункт 1, и настоящим разделом не отменяется.
...
Шесть мест, а не четыре (раздел 2). Шестое добавилось из-за того, что режим стал долговременным
свойством базы, — см. ниже:
